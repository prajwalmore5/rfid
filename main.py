...
t13 = now.replace(hour = 2, minute = 45, second = 0, microsecond = 0)
t14 = now.replace(hour = 3, minute = 10, second = 0, microsecond = 0)
t15 = now.replace(hour = 4, minute = 0, second = 0, microsecond = 0)
t16 = now.replace(hour = 4, minute = 10, second = 0, microsecond = 0)
  ...
  while True:
    ...
    if status == MIFAREReader.MI_OK :
      ...
      column_number = cell.col #store value of column
        ...
        if now <= t2:
          if value == check_uid:
          sheet.update_cell(row_number,lec,'present')
          print('done')
